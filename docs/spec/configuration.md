...
| `statusIntervalSeconds` | `30` | Interval between status reports |
...
| `mockMode` | `false` | Presentation: fix `merchantId`/`orderType`/`orderId` to `mock` and randomize `displayId`/`pickupCode` on each print |
...
|----------|-------------|
| `IFOOD_QR_SCRIPTS_DIR` | Override scripts directory (used by `raw-forwarder.ts`) |
